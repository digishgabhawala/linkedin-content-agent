...
clarify → draft (local LLM) → lock → on-brand character illustration →
manual review and posting. Uses a consistent mascot character for visual
branding, generated by the sibling project
...
when it's done). Everything else -- understanding the brief, writing the
post, deriving what the illustration should show -- runs against a local
Ollama model, no cloud dependency.
...
1. You tell it, in your own words, what you did.
2. If the brief is thin, it asks a focused follow-up question to get at
   ...
   weren't given to it. **Current limitation:** clarification is capped at 3
   questions before it drafts anyway regardless of quality; the intended
   behavior (removing that cap in favor of continuing until the post has
   enough real material, or the user says there's nothing more to add) is
   backlog, see Upcoming.
...
| `CLARIFY_MAX_TURNS` | `3` | current hard cap on clarify questions -- see Upcoming |
...
- **Remove the hard clarify-question cap.** Currently forced to a draft
  after 3 questions regardless of whether the post has enough real material.
  This was never an intentional design goal -- it should keep asking until
  either the draft would genuinely be good, or the user says there's nothing
  more to add.
...
- **Generalize beyond the current test persona** (see the note at the top) --
  nothing in the pipeline is hardcoded to one voice or industry today, but
  it also hasn't been validated against any other kind of user yet.
